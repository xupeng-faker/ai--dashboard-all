-- =============================================================================
-- 个人训战 / 课程完成相关表：若不存在则创建，并补齐最小演示数据（可重复执行）
-- 解决：Table 'ai_transform.ai_course_planning_info' doesn't exist
-- 执行：mysql -h127.0.0.1 -uroot -p ai_transform < src/main/resources/db/ensure_personal_course_tables.sql
-- =============================================================================
...
-- 课程规划信息表
CREATE TABLE IF NOT EXISTS `ai_course_planning_info` (
...
-- 部门选课信息表
CREATE TABLE IF NOT EXISTS `dept_course_selections` (
  `id` int(11) NOT NULL AUTO_INCREMENT COMMENT '主键ID',
...
-- 员工同步表 (用于个人课程查询)
CREATE TABLE IF NOT EXISTS `t_employee_sync` (
...
-- 微课学习记录同步表
CREATE TABLE IF NOT EXISTS `t_micro_study_info_sync` (
...
-- MOOC学习记录同步表
CREATE TABLE IF NOT EXISTS `t_mooc_study_info_sync` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT,
  `emp_num` varchar(50) DEFAULT NULL COMMENT '工号',
  `course_team_code` varchar(100) DEFAULT NULL COMMENT '课程编码',
  `is_pass` varchar(10) DEFAULT NULL COMMENT '是否通过（1-通过）',
...
ON DUPLICATE KEY UPDATE
  ...
  big_type = VALUES(big_type),
  course_link = VALUES(course_link),
  credit = VALUES(credit);

-- 演示员工（无则插入一条，避免重复）
INSERT INTO `t_employee_sync` (`employee_number`, `last_name`, `fourthdeptcode`)
SELECT 'E001234', '张三', '3001' FROM DUAL
WHERE NOT EXISTS (SELECT 1 FROM `t_employee_sync` WHERE `employee_number` = 'E001234' LIMIT 1);

-- 部门选课（与 CoursePlanning 逻辑一致：目标课 id 1~7）
...
-- 张三部分完课记录（重复执行脚本可能多行，查询用 DISTINCT 不受影响；如需干净可先手动删该工号学习记录）
