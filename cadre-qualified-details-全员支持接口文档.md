# /cadre-qualified-details 接口扩展支持全员数据查询

## 一、接口概述

### 1.1 接口信息
- **接口路径**: `/expert-cert-statistics/cadre-qualified-details`
- **请求方式**: `GET`
- **接口描述**: 查询干部/专家/全员任职详细信息，支持按AI成熟度、职位类等条件筛选

### 1.2 当前支持情况
- ✅ **personType=1**: 干部数据查询（已支持）
- ✅ **personType=2**: 专家数据查询（已支持）
- ❌ **personType=0**: 全员数据查询（**本次新增支持**）
...
| queryType | Integer | 否 | 查询类型：1-任职人数（默认），2-基线人数。全员、干部、专家类型都支持 |
...
#### queryType
- **1**: 任职人数（默认值）
  - 全员类型：只返回已获得AI任职的员工详细信息（INNER JOIN t_qualifications）
  - 干部类型：只返回已获得AI任职的干部详细信息
  - 专家类型：只返回已获得AI任职的专家详细信息
- **2**: 基线人数
  - 全员类型：返回所有员工信息（不限制是否任职），如果有任职信息则一并返回（LEFT JOIN t_qualifications）
  - 干部类型：返回所有干部（不限制是否任职）
  - 专家类型：返回所有专家（不限制是否任职）
...
当 `personType=0` 时，按以下步骤查询（**与employee-cert-statistics接口的查询逻辑一致**）：

1. **参数校验**
   - 如果 `queryType` 为空或未提供，默认为1（任职人数）
   - 如果 `queryType` 不为1且不为2，返回错误："查询类型参数错误，只支持1（任职人数）或2（基线人数）"
...
2. **特殊处理：当 deptCode 为 "0" 时**
   ...
   - 查询云核心网产品线部门下的所有四级部门
   - 如果没有四级部门，返回空列表
   - 设置 `queryLevel = 4`（用于后续查询四级部门的员工）
   - 跳转到步骤4，遍历这些四级部门

3. **普通处理：当 deptCode 不为 "0" 时**
   - **3.1 查询部门信息**
...
4. **查询员工任职详细信息**
   - 对于当前部门（或四级部门列表），执行以下步骤：
     ...
     - **4.3 查询员工任职详细信息**
       ...
       - **当 queryType=1（任职人数）时**：
         - 使用 INNER JOIN 关联 `t_qualifications` 表
         - 只返回已获得AI任职的员工详细信息
         - 任职筛选条件：
           - 方向名称包含AI相关方向：
             - `数据科学与AI工程（ICT）`
         ...
         - 如果同一员工有多条有效的AI任职记录，选择最高级别的记录
       - **当 queryType=2（基线人数）时**：
         - 使用 LEFT JOIN 关联 `t_qualifications` 表
         - 返回所有员工信息（不限制是否任职）
         - 如果员工有任职信息，则一并返回任职详细信息
         - 如果员工没有任职信息，任职相关字段（competenceFamilyCn、competenceCategoryCn等）返回 null
         - 如果员工有多条有效的AI任职记录，选择最高级别的记录
       - 需要查询的字段包括：
         - 基本信息：姓名、工号、职位类、职位子类
         - 部门信息：一级到六级部门名称
         - 任职信息：能力族、能力类、能力子类、方向名称、能力等级、能力级别、任职开始时间、任职结束时间（queryType=2时可能为null）
         - 其他信息：是否为干部、干部类型、最小部门名称、岗位AI成熟度（全员类型可能为空）

5. **构建返回结果**
   - `employeeDetails`: 包含所有符合条件的员工任职详细信息列表
   - 去重（按工号去重，如果同一员工有多条任职记录，保留最高级别的记录）

### 3.2 任职详细信息查询逻辑

#### 3.2.1 数据来源
- 员工基本信息：`t_employee_sync` 表（`period_id = 20251126`）
- 任职信息：`t_qualifications` 表
- 干部信息：`t_cadre` 表（LEFT JOIN，用于判断是否为干部、获取干部类型和干部岗位成熟度）
...
- SQL示例：
  ...
  ```sql
  CASE 
      WHEN exp.position_ai_maturity IS NOT NULL THEN exp.position_ai_maturity
      WHEN c.position_ai_maturity IS NOT NULL THEN c.position_ai_maturity
      ELSE NULL
  END AS aiMaturity
  ```

#### 3.2.2 任职记录选择规则
- 如果同一员工有多条有效的AI任职记录，应选择最高级别的记录
- 级别排序规则（从高到低）：
  - 8级 > 7级 > 6级 > 5级 > 4级 > 3级 > 2级 > 1级 > 初级
- 如果级别相同，选择 `competence_from` 最新的记录
...
| competenceSubcategory | t_employee_sync.job_category（提取子类部分） | 职位子类 |
...
| competenceFamilyCn | t_qualifications.competence_family_cn | 能力族 |
| competenceCategoryCn | t_qualifications.competence_category_cn | 能力类 |
| competenceSubcategoryCn | t_qualifications.competence_subcategory_cn | 能力子类 |
| directionCnName | t_qualifications.direction_cn_name | 方向中文名称 |
| competenceRatingCn | t_qualifications.competence_rating_cn | 能力等级中文 |
| competenceGradeCn | t_qualifications.competence_grade_cn | 能力级别中文 |
| competenceFrom | t_qualifications.competence_from | 任职开始时间 |
| competenceTo | t_qualifications.competence_to | 任职结束时间 |
...
| isQualificationsStandard | NULL（全员类型不查询达标情况） | 是否按要求任职达标 |
| isCertStandard | NULL（全员类型不查询达标情况） | 是否认证达标 |
...
#### 新增：查询全员任职详细信息的SQL（EmployeeMapper.xml）
...
<select id="getEmployeeQualifiedDetailsByDeptLevel" resultType="com.huawei.aitransform.entity.EmployeeDetailVO">
    SELECT DISTINCT
        ...
        -- 职位子类可以从job_category中进一步提取，如果需要的话
        NULL AS competenceSubcategory,
    ...
    <choose>
        <when test="queryType != null and queryType == 2">
            ...
            LEFT JOIN (
                SELECT 
                    q.employee_number,
                    q.competence_family_cn,
                    q.competence_category_cn,
                    q.competence_subcategory_cn,
                    q.direction_cn_name,
                    q.competence_rating_cn,
                    q.competence_grade_cn,
                    q.competence_from,
                    q.competence_to,
                    ROW_NUMBER() OVER (
                        PARTITION BY q.employee_number 
                        ORDER BY 
                            CASE 
                                -- 8级最高，依次到1级，初级最低
                                WHEN q.competence_rating_cn = '8级' THEN 1
                                WHEN q.competence_rating_cn = '7级' THEN 2
                                WHEN q.competence_rating_cn = '6级' THEN 3
                                WHEN q.competence_rating_cn = '5级' THEN 4
                                WHEN q.competence_rating_cn = '4级' THEN 5
                                WHEN q.competence_rating_cn = '3级' THEN 6
                                WHEN q.competence_rating_cn = '2级' THEN 7
                                WHEN q.competence_rating_cn = '1级' THEN 8
                                WHEN q.competence_rating_cn = '初级' THEN 9
                                ELSE 10
                            END,
                            q.competence_from DESC
                    ) AS rn
                FROM t_qualifications q
                WHERE q.employee_number IS NOT NULL
            ...
            ) q_ranked ON (e.employee_number = q_ranked.employee_number AND q_ranked.rn = 1)
            LEFT JOIN t_qualifications q ON (
                e.employee_number = q.employee_number
                AND q.employee_number = q_ranked.employee_number
                AND q.competence_from = q_ranked.competence_from
            )
        </when>
        <otherwise>
...
**说明**：
- 该SQL参考 `ExpertCertStatisticsMapper.getExpertQualifiedDetailsByConditions` 的实现逻辑
- 区别在于：
  - **需要关联 `t_expert` 表**（LEFT JOIN）：用于判断是否为专家、获取专家岗位成熟度
  - **需要关联 `t_cadre` 表**（LEFT JOIN）：用于判断是否为干部、获取干部类型和干部岗位成熟度
  ...
  - **queryType参数处理**：
    - `queryType=1`：使用 INNER JOIN，只返回已获得AI任职的员工
    - `queryType=2`：使用 LEFT JOIN，返回所有员工，如果有任职信息则一并返回
  - **岗位成熟度获取逻辑**：
    - **专家和干部是互斥关系**：如果员工是专家，就一定不会是干部；如果员工是干部，就一定不会是专家
        ...
        "employeeDetails": [
            {
                ...
                "competenceSubcategory": null,
                "firstLevelDept": "一级部门",
                "secondLevelDept": "二级部门",
                "thirdLevelDept": "三级部门",
                "fourthLevelDept": "四级部门",
                "fifthLevelDept": "五级部门",
                "sixthLevelDept": "六级部门",
                "competenceFamilyCn": "能力族名称",
                "competenceCategoryCn": "能力类名称",
                "competenceSubcategoryCn": "能力子类名称",
                "directionCnName": "AI算法及应用（ICT）",
                "competenceRatingCn": "5级",
                "competenceGradeCn": "专业级",
                "competenceFrom": "2024-01-01",
                "competenceTo": "2025-12-31",
                "isCadre": 0,
                "cadreType": null,
                "miniDeptName": "最小部门名称",
                "aiMaturity": null,
...
| 字段名 | 类型 | 说明 |
|--------|------|------|
| employeeDetails | List | 员工任职详细信息列表 |
| name | String | 姓名 |
| employeeNumber | String | 工号 |
| competenceCategory | String | 职位类（从job_category字段提取） |
| competenceSubcategory | String | 职位子类（可能为null） |
| firstLevelDept | String | 一级部门名称 |
| secondLevelDept | String | 二级部门名称 |
| thirdLevelDept | String | 三级部门名称 |
| fourthLevelDept | String | 四级部门名称 |
| fifthLevelDept | String | 五级部门名称 |
| sixthLevelDept | String | 六级部门名称 |
| competenceFamilyCn | String | 能力族 |
| competenceCategoryCn | String | 能力类 |
| competenceSubcategoryCn | String | 能力子类 |
| directionCnName | String | 方向中文名称（AI相关方向） |
| competenceRatingCn | String | 能力等级中文（如：5级） |
| competenceGradeCn | String | 能力级别中文（如：专业级） |
| competenceFrom | Date | 任职开始时间 |
| competenceTo | Date | 任职结束时间 |
...
| isCertStandard | Integer | 是否认证达标（全员类型为null） |

### 4.3 全员数据查询的特殊说明

- **employeeDetails**: 包含所有符合条件的员工任职详细信息
- **任职记录选择**: 如果同一员工有多条有效的AI任职记录，只返回最高级别的记录
...
{
    "code": 400,
    "message": "查询类型参数错误，只支持1（任职人数）或2（基线人数）",
    "data": null
}
```
...
**请求示例**:
```
GET /expert-cert-statistics/cadre-qualified-details?deptCode=123456&personType=0&queryType=1
```
        ...
        "employeeDetails": [
            {
                ...
                "competenceCategory": "软件类",
                "competenceSubcategory": null,
                "firstLevelDept": "云核心网产品线",
                "secondLevelDept": "产品开发部",
                "thirdLevelDept": "软件开发部",
                "fourthLevelDept": "AI算法组",
                "fifthLevelDept": null,
                "sixthLevelDept": null,
                "competenceFamilyCn": "AI算法",
                "competenceCategoryCn": "机器学习",
                "competenceSubcategoryCn": "深度学习",
                "directionCnName": "AI算法及应用（ICT）",
                "competenceRatingCn": "5级",
                "competenceGradeCn": "专业级",
                "competenceFrom": "2024-01-01T00:00:00",
                "competenceTo": "2025-12-31T00:00:00",
                "isCadre": 0,
                "cadreType": null,
                "miniDeptName": "AI算法组",
                "aiMaturity": null,
...
**请求示例**:
```
GET /expert-cert-statistics/cadre-qualified-details?deptCode=123456&personType=0&queryType=1&jobCategory=软件类
```
...
**请求示例**:
```
GET /expert-cert-statistics/cadre-qualified-details?deptCode=123456&personType=0&queryType=2
```
    ...
    "data": {
        "employeeDetails": [
            ...
            },
            {
                ...
                "fifthLevelDept": null,
                "sixthLevelDept": null,
                "competenceFamilyCn": null,
                "competenceCategoryCn": null,
                "competenceSubcategoryCn": null,
                "directionCnName": null,
                "competenceRatingCn": null,
                "competenceGradeCn": null,
                "competenceFrom": null,
                "competenceTo": null,
                "isCadre": 0,
                "cadreType": null,
...
- 张三有任职信息，返回完整的任职详细信息
- 李四没有任职信息，任职相关字段（competenceFamilyCn、directionCnName等）为 null

### 6.4 查询全员任职数据（deptCode=0，查询所有四级部门）

**请求示例**:
```
GET /expert-cert-statistics/cadre-qualified-details?deptCode=0&personType=0&queryType=1
```
...
- ✅ `EmployeeMapper.getEmployeeNumbersByDeptLevel()` - 查询员工工号列表（已存在，但需要修改为支持单个部门查询）
- ✅ `ExpertCertStatisticsService.getQualifiedEmployeeNumbers()` - 查询任职人数（已存在）

### 7.2 需要新增的代码

#### Mapper层
- **EmployeeMapper接口**: 新增方法 `getEmployeeQualifiedDetailsByDeptLevel()`
  ```java
  ...
  **注意**：
  ...
  - 新增 `queryType` 参数，用于控制查询逻辑（1-任职人数，2-基线人数）

- **EmployeeMapper.xml**: 新增SQL查询方法 `getEmployeeQualifiedDetailsByDeptLevel`
  - 参考 `ExpertCertStatisticsMapper.getExpertQualifiedDetailsByConditions` 的实现
  - 关联表：
    - `t_employee_sync` 表（主表）
    - `t_qualifications` 表（任职信息，根据 queryType 决定 JOIN 方式）
    - `t_cadre` 表（LEFT JOIN，用于判断是否为干部、获取干部类型和干部岗位成熟度）
...
#### Service层
- **ExpertCertStatisticsService.getCadreQualifiedDetailsByConditions()**
  - 添加 `personType=0` 的处理逻辑
  - **参数校验**：
    - 如果 `queryType` 不为1且不为2，抛出异常："查询类型参数错误，只支持1（任职人数）或2（基线人数）"
  ...
  - **特殊处理：当 deptCode="0" 时**：
    ...
    - 如果没有四级部门，返回空列表
    - **遍历每个四级部门**：
      - 调用 `EmployeeMapper.getEmployeeQualifiedDetailsByDeptLevel(queryLevel, dept.getDeptCode(), jobCategory, queryType)` 查询该部门的员工任职详细信息
      - 合并所有四级部门的查询结果
  - **普通处理：当 deptCode 不为 "0" 时**：
    - 查询当前部门信息，获取部门层级（`deptLevel`）
    - 直接使用当前部门的层级进行查询（`queryLevel = currentLevel`）
    - 调用 `EmployeeMapper.getEmployeeQualifiedDetailsByDeptLevel(queryLevel, deptCode, jobCategory, queryType)` 查询当前部门的员工任职详细信息
    - 该方法内部已经处理了：
      - 按部门层级过滤（使用当前部门的层级）
      - 按职位类过滤（如果提供了jobCategory参数）
      - **根据 queryType 决定返回范围**：
        - `queryType=1`：只返回已获得任职的员工（INNER JOIN t_qualifications）
        - `queryType=2`：返回所有员工，如果有任职信息则一并返回（LEFT JOIN t_qualifications）
      - 选择每个员工的最高级别任职记录（如果有多条记录）
  - **构建返回结果**：
    - 返回格式与干部/专家查询完全一致

#### Controller层
- **ExpertCertStatisticsController.getCadreQualifiedDetailsByConditions()**
...
1. **queryType参数处理**: 
   - 全员类型支持 `queryType=1`（任职人数）和 `queryType=2`（基线人数）
   - `queryType=1`：只返回已获得AI任职的员工详细信息（使用 INNER JOIN）
   - `queryType=2`：返回所有员工信息，如果有任职信息则一并返回（使用 LEFT JOIN）
...
4. **部门层级处理**: 
   ...
   - 使用当前部门的层级（`queryLevel = currentLevel`）调用 `getEmployeeQualifiedDetailsByDeptLevel()`
   - 注意：`queryLevel` 是当前部门的层级，用于确定查询 `t_employee_sync` 表的哪个部门字段
   - SQL中使用 `=` 精确匹配当前部门编码，而不是 `IN` 查询多个部门

5. **任职记录选择**: 
   - 如果同一员工有多条有效的AI任职记录，应选择最高级别的记录
   - 级别排序：8级 > 7级 > 6级 > 5级 > 4级 > 3级 > 2级 > 1级 > 初级

6. **数据一致性**: 
   - 当 `queryType=1` 时，查询的人员全集与 `employee-cert-statistics` 接口一致（只返回已获得任职的员工）
   - 当 `queryType=2` 时，返回所有员工信息（不限制是否任职），与 `employee-cert-statistics` 接口的基线人数统计逻辑一致
   - 都使用 `period_id = 20251126` 过滤
   - 都使用相同的部门层级查询逻辑
   - 任职判断标准一致（AI相关方向、有效期等）

7. **空数据处理**: 
   - 如果当前部门下没有符合条件的员工，返回空列表
   - 当deptCode="0"时，如果某个四级部门下没有符合条件的员工，该部门不返回数据
...
### 8.1 功能测试
1. 测试 `personType=0` 时，能正确查询全员任职数据
2. 测试不同部门层级的全员查询
3. 测试 `deptCode="0"` 时，能正确查询所有四级部门的全员数据
4. 测试按职位类过滤功能
5. 测试 `queryType=1` 时，只返回已获得任职的员工
6. 测试 `queryType=2` 时，返回所有员工信息（包括没有任职的员工）
7. 测试 `queryType=2` 时，有任职信息的员工返回任职详细信息，没有任职信息的员工任职相关字段为null
...
5. 测试同一员工有多条任职记录时，是否正确选择最高级别的记录

### 8.3 对比测试
1. 对比 `employee-cert-statistics` 接口查询的任职人数，确保一致性
2. 对比任职详细信息与 `t_qualifications` 表数据的一致性
3. 验证职位类过滤的准确性
4. 验证部门层级查询的准确性
